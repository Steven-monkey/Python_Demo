...
def inputbox(showstr,showorder,length):
    instr=input(showstr)            #使用input函数要求用输入信息，showstr位输入提示文字
    if len(instr)!=0:               #输入数据的长度不为0
        ...
        if showorder==2:
            if str.isalpha(instr):
                if len(instr)!=length:
                    print('\033[1;31;40m必须重新输入'+str(length)+'个字母，请重新输入！！\033[0m')
                    return '0'
                else:
                    return instr
            else:
        ...
        if showorder==3:
            if str.isdigit(instr):
                if len(instr)!=length:
                    print('\033[1;31;40m必须重新输入'+str(length)+'个数字，请重新输入！！\033[0m')
                    return '0'
                else:
                    return instr
            else:
...
def scode4(schoice):
    ...
    while not str.isalpha(intype) or len(intype) != 3:
        intype=inputbox('\033[1;32m     请输入要生成的带数据分析功能的防伪码数量：\33[0m',1,0)
    incount=inputbox("\033[1;32m        请输入要生成的25位混合产品序列号数量:\33[0m",1,0)
    while int(incount)==0:
        incount=inputbox("\033[1;32m        请输入要生成的25位混合产品序列号数量:\33[0m",1,0)
...
#智能批量生成带数据分析功能的防伪码
def scode5(schoice):
    default_dir=r'codeauto.mri' #设置默认打卡的文件名
    file_path=tkinter.filedialog.askopenfilename(filetypes=[("Text file",".mri")],
    title=u'请选择智能批量处理文件：',initialdir=(os.path.expanduser(default_dir)))
...
#实现防伪码补充生成功能
def scode6(schoice):
    default_dir=r'ABCscode.txt'
    file_path=tkinter.filedialog.askopenfilename(title=u'请选择已经生成的防伪文件',initialdir=(os.path.expanduser(default_dir)))
    codelist=openfile(file_path)
    ...
    nres_letter=strpro.replace(''''','').replace(''''' , '') + strtype.replace(''''','').replace(''''' , '')
...
#实现条形码输出
def scode7(schoice):
    mainid=inputbox('\033[1,32m     请输入数据分析编号（3位字母）:\33[0m',1,0)
